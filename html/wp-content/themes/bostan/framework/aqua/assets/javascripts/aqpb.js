...
jQuery(document).ready(function($){
		...
		resizable_args = {
			...
			resize: function(event, ui) {
			    ...
			    ui.helper.css("margin-left", ui.position.left + 20 + "px");
					ui.helper.find('> div > .size').val(block_size( ui.size.width ));
			},
			stop: function(event, ui) {
				...
				ui.helper.find('.block-width select').val(block_size( $(ui.helper).css('width') ));
	...
	function block_width_size(width) {
		var span = '100%';

		if (width == 'span2'){ span = "16.67%"; }
		else if (width == 'span3'){ span = "25%"; }
		else if (width == 'span4'){ span = "33.33%"; }
		else if (width == 'span5'){ span = "41.67%"; }
		else if (width == 'span6'){ span = "50%"; }
		else if (width == 'span7'){ span = "58.83%"; }
		else if (width == 'span8'){ span = "66.66%"; }
		else if (width == 'span9'){ span = "75%"; }
		else if (width == 'span10'){ span = "83.33%"; }
		else if (width == 'span11'){ span = "91.67%"; }
		else if (width == 'span12'){ span = "100%"; }
	...
	$('ul.blocks li.block').each(function() {
		var blockID = $(this).attr('id'),
			...
			var blockItem = $('#' + blockID);
			//blocks resizing
			blockItem.resizable(resizable_args);
			blockItem.on('mousedown', '.ui-resizable-handle', function (e) {
				$('#' + blockID + ' .block-width select').val(block_size( blockItem.css('width')));
			});
			$('#' + blockID + ' .block-width select').change( function () {
				var value = $(this).val();
				$('#' + blockID).removeClass(function (index, css) {
						return (css.match (/\bspan\S+/g) || []).join(' ');
				}).addClass(value);
				blockItem.find('> div > input.size').val(value);
				blockItem.css('width', block_width_size(value));
			});
	...
	$( "ul.blocks" ).bind( "sortstop", function(event, ui) {
		...
		if (ui.item.hasClass('ui-draggable')) {
		    ...
		    ui.item.find('a.block-edit').click();
				blockItem.css('width', block_width_size($('#' + blockID + ' .block-width select').val()));
		    //disable resizable on .not-resizable blocks
		    $(".ui-resizable.not-resizable").resizable("destroy");
