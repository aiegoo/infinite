...
if(!class_exists('AQ_Block')) {
	class AQ_Block {
	 	...
	 	function before_form($instance) {
			...
			$other_size_array = array('span2' => '16.67%',
																'span5' => '41.67%',
																'span7' => '58.83%',
																'span10' => '83.33%',
																'span11' => '91.67%'
			...
			$block_width_list = '';
		  foreach ($size_array as $sizevalue => $sizename) {
				$selected = (strpos($size, $sizevalue) !== false) ? 'selected' : '';
				$block_width_list .= '<option value="'.$sizevalue.'" '.$selected.'>'.$sizename.'</option>';
		  }
			$block_width_list .= '<optgroup label="Other">';
			foreach ($other_size_array as $sizevalue => $sizename) {
				$selected = (strpos($size, $sizevalue) !== false) ? 'selected' : '';
				$block_width_list .= '<option value="'.$sizevalue.'" '.$selected.'>'.$sizename.'</option>';
			}
			$block_width_list .= '</optgroup>';
	 				...
	 				'<dl class="block-bar">',
	 					'<dt class="block-handle">',
	 						'<div class="block-title">',
	 							$name , $title,
	 						'</div>',
							'<span class="block-width"><select>'.$block_width_list.'</select></span>',
	 	...
	 	function before_block($instance) {
			...
			if ($missedlast == true) {
	 		...
	 		if ($first == true) {
				if (isset($thewidth) && $thewidth == 'fluid' && $size == 'span12') {
				echo '<div class="container-fluid new_section fullwidth_'.$id_base.'"><div class="row-fluid">';
				}else{
				echo '<div class="container-fluid new_section"><div class="container"><div class="row-fluid">';
				}
			}
	 		echo '<div id="aq-block-'.$number.'" class="aq-block  aq-block-'.$id_base.' aq_'.$size.' '.$size.' '.$column_class.' cf">';
	 	}
