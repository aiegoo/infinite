		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'multiselect':
					...
					$output .= '<select multiple class="select of-input" name="'.$value['id'].'[]" id="'. $value['id'] .'">';

					foreach ($value['options'] as $select_ID => $option) {
						$theValue = $option;
						if (!is_numeric($select_ID))
							$theValue = $select_ID;
							if (is_array($smof_data[$value['id']])) {
								$selected = (in_array($theValue, $smof_data[$value['id']]) !== false) ? 'selected' : '' ;
							} else {
								$selected = '';
							}
