<?php
	...
	*/
	...
	if( !is_admin() ){ 
		...
		add_action('wp_enqueue_scripts', 'gdlr_core_deregister_front_script', 15); 
	}
	...
	if( !function_exists('gdlr_core_deregister_front_script') ){
		function gdlr_core_deregister_front_script( $admin = false ){
			if( !$admin ){
				wp_deregister_style('font-awesome');
				wp_deregister_style('font-elegant');
				wp_deregister_style('elegant-font');
			}
		}
	}
